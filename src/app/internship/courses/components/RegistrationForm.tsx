...
export function RegistrationForm({ selectedCourseSlug, selectedCourseId, selectedPlan, plans, loadingPlans }: RegistrationFormProps) {
  ...
  useEffect(() => {
    if (!selectedCourseSlug || !session) return;
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      if (!selectedCourseSlug || !selectedPlan) {
        toast.error("Please select a course and a plan.");
        return;
      }

      // We must have a numeric ID for the backend APIs.
      const courseIdToSubmit = selectedCourseId;

      if (!courseIdToSubmit) {
        toast.error("Course ID missing, please refresh page.");
        return;
      }
      ...
      try {
        ...
        const orderRes = await fetch("/api/razorpay/order", {
          ...
          body: JSON.stringify({
            courseId: courseIdToSubmit, // Sending numeric ID
            planId: selectedPlan,
          ...
          handler: async function (response: any) {
            ...
            try {
              ...
              if (verifyData.success) {
                toast.success("Enrollment Successful!");
                setIsSuccess(true);
                // router.push("/internship/courses/" + selectedCourseSlug); // Redirect to course page using slug
              } else {
      ...
      <Card className="bg-green-50 border-green-200">
        ...
        <CardContent>
          <Button asChild className="w-full bg-green-600 hover:bg-green-700" size="lg">
            <Link href={`/internship/courses/${selectedCourseSlug}`}>Start Learning Now</Link>
          </Button>
        </CardContent>
      </Card>
    );
      ...
      <Card className="bg-blue-50 border-blue-200">
        ...
        <CardContent>
          <Button asChild className="w-full bg-blue-600 hover:bg-blue-700" size="lg">
            <Link href={`/internship/courses/${selectedCourseSlug}`}>Go to Course</Link>
          </Button>
        </CardContent>
      </Card>
    );
        ...
        >
          ...
          <div className="pt-2">
            ...
            <Button
              ...
              disabled={!REGISTRATION_OPEN || form.state.isSubmitting || !selectedCourseSlug || !selectedPlan}
            >
              {form.state.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {!REGISTRATION_OPEN ? "Registrations Closed" : !selectedCourseSlug ? "Select a Course" : !selectedPlan ? "Select a Plan" : `Pay & Register (₹${finalPrice})`}
            </Button>
          </div>
        </form>
      </CardContent>
